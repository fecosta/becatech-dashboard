...
| 4 · Vulnerability level | **Implemented** | the cross-country reconciliation turns out to be done upstream — one harmonised scale already. The naming was the blocker: the design relabelled the lowest band "Vulnerable", reversing what that row asserts. SPEC-004 D4 resolves it by restating the source values (High / Moderate / Low vulnerability) — see `lib/scholars/vulnerability-level.ts` |
