// Source adapter for the "MENTOR REPORTS" tab — flat, one row per mentor session, already
// carrying a Submission ID column (unlike SCHOLAR GENERAL INFO / SUPPORT ACTIVITY LOG, there is
// no repeating-period block to pivot here). The real header sits a few rows below a decorative
// mini pivot-table summary, so it's located dynamically instead of assuming row 1. Formerly
// adapters/legacy-mentor-reports.ts (now a thin re-export of this module, kept for import-path
// compatibility) — moved to its canonical name per docs/adr/007-spreadsheet-source-adapters.md.
import * as XLSX from "xlsx";
import { coerceValue } from "../coerce";
import type { ParsedSheet } from "../parse";
import { mentorReportsContract } from "../source-contracts/mentor-reports";
import type { CanonicalBatch, CanonicalRow, FieldType, SourceAdapter } from "../types";
...
export function mentorReportsLegacyAdapter(sheet: ParsedSheet): CanonicalRow[] {
  ...
  records.forEach((rec, i) => {
    ...
    // "scholar's name" is the intended new-sheet header, but the live sheet actually has it as
    // "SCHOLAR'NAME" (missing the S, no space before "name") — confirmed from production. Kept as
    // an explicit alias, not a fuzzy apostrophe-tolerant match, since that's the only variant seen.
    const scholarName = c(getAny(idx, ["nombre del becario", "scholar's name", "scholar'name"]), "string");
    ...
    out.push({
      ...
      data: {
        ...
        mentorName: c(getAny(idx, ["soy:", "mentor's name", "mentor' s name"]), "string"),
...
export const mentorReportsAdapter: SourceAdapter<ParsedSheet> = {
  source: "MENTOR_REPORTS",
  canHandle: isMentorReportsSheet,
  adapt: (sheet) => {
    const rows = mentorReportsLegacyAdapter(sheet);
    const batch: CanonicalBatch = {};
    if (rows.length > 0) batch.MENTOR_REPORT = rows;
    return batch;
  },
  inspectSchema: (sheet) => {
    const headerRowIndex = findHeaderRowIndex(sheet);
    const headerKeys = headerRowIndex >= 0 ? (rawRows(sheet)[headerRowIndex - sheetStartRow(sheet)] ?? []).map(normKey) : [];
    return classifyColumns(headerKeys, mentorReportsContract);
  },
};
